...
# Source all slide files
source("slides/login_slide.R")
source("slides/welcome_slide.R")
source("slides/top_track_slide.R")
source("slides/top_tracks_slide.R")
source("slides/top_artist_slide.R")
source("slides/top_artists_slide.R")
source("slides/popularity_slide.R")
source("slides/top_1_genre_slide.R")
source("slides/genres_slide.R")
source("slides/moods_slide.R")
source("slides/personality_slide.R")
source("slides/personality_details_slide.R")
source("slides/thank_you_slide.R")
...
function(input, output, session) {
  ...
  slides <- list(
    list(id = "login", title = "Welcome to Your 2025 Spotify Wrap"),
    ...
    list(id = "thank-you", title = "That's Your 2025 Wrap!")
  )
  ...
  fetchMoodDistribution <- function() {
    # API call to fetch mood distribution
    if (is.null(values$username) || is.null(values$filename)) return()
  ...
  render_wordcloud_slide <- function() {
    if (!values$logged_in) {
      return(div("Please log in first"))
    }
    ...
    if (!values$data_loaded || is.null(values$top_tracks)) {
      return(div(style = "text-align: center; padding: 4rem;",
        ...
        h3("Configure Your Analysis First", style = "color: #1DB954; margin-bottom: 1rem; font-size: 1.8rem; font-family: 'Montserrat', sans-serif;"),
        p("Please go back to 'Your Musical Journey' and configure your preferences to load your tracks.",
          style = "color: #D1D5DB; font-size: 1.2rem; max-width: 500px; margin: 0 auto; font-family: 'Montserrat', sans-serif;"),
        div(style = "margin-top: 2rem;",
          actionButton("back_to_config_wordcloud", "← Back to Configuration", 
            style = "background: #1DB954; color: white; border: none; padding: 0.75rem 1.5rem; border-radius: 1rem; cursor: pointer; font-family: 'Montserrat', sans-serif;"
          )
        )
      ))
    ...
    if (length(tracks) == 0) {
      return(div(style = "text-align: center; padding: 4rem;",
        div(style = "font-size: 3rem; margin-bottom: 1rem;", "☁️"),
        p("No tracks found for wordcloud. Try listening to more music on Spotify!", 
          style = "color: #D1D5DB; font-size: 1.2rem; font-family: 'Montserrat', sans-serif;")
      ))
    }

    # Extract song names and clean them
    song_names <- c()
    for (i in seq_along(tracks)) {
      ...
      if (!is.null(track$name)) {
        ...
        # Clean the track name for wordcloud
        # Remove common words, special characters, and make it wordcloud-friendly
        cleaned_name <- gsub("[^A-Za-z0-9 ]", "", track_name)
        cleaned_name <- gsub("\\s+", " ", cleaned_name)
        
        # Split into words and filter out common stop words
        words <- unlist(strsplit(cleaned_name, " "))
        words <- words[nchar(words) > 2]  # Remove very short words
        
        # Remove common stop words
        stop_words <- c("the", "and", "you", "are", "for", "with", "not", "have", "this", "that", "was", "but", "his", "her", "she", "him", "can", "had", "what", "one", "our", "out", "day", "get", "use", "man", "new", "now", "way", "may", "say")
        words <- words[!tolower(words) %in% stop_words]
        
        song_names <- c(song_names, words)
      }
    }
    
    # Create frequency table for wordcloud
    if (length(song_names) > 0) {
      # Create word frequency
      word_freq <- table(tolower(song_names))
      ...
      if (length(word_freq) > 0) {
        ...
        wordcloud_data <- wordcloud_data[order(-wordcloud_data$freq), ]
        wordcloud_data <- wordcloud_data[1:min(100, nrow(wordcloud_data)), ]
        
        div(style = "max-width: 1000px; margin: 0 auto;",
          div(style = "text-align: center; margin-bottom: 3rem;",
            p("The words that define your musical universe", style = "font-size: 1.5rem; color: #D1D5DB; font-family: 'Montserrat', sans-serif;")
          ),
          
          # Wordcloud container
          div(style = "background: rgba(0,0,0,0.3); border-radius: 1.5rem; padding: 2rem; border: 2px solid rgba(29, 185, 84, 0.3); box-shadow: 0 8px 32px rgba(29, 185, 84, 0.2);",
            wordcloud2Output("song_wordcloud", height = "500px", width = "100%")
          ),
          
          # Stats below wordcloud
          div(style = "text-align: center; margin-top: 2rem;",
            div(class = "stat-grid", style = "display: flex; justify-content: center; gap: 3rem; flex-wrap: wrap;",
              div(class = "stat-card", style = "text-align: center;",
                div(class = "stat-value", style = "font-size: 2rem; font-weight: bold; color: #1DB954; font-family: 'Montserrat', sans-serif;", length(tracks)),
                div(class = "stat-label", style = "font-size: 1rem; color: #9CA3AF; font-family: 'Montserrat', sans-serif;", "Songs Analyzed")
              ),
              div(class = "stat-card", style = "text-align: center;",
                div(class = "stat-value", style = "font-size: 2rem; font-weight: bold; color: #8B5CF6; font-family: 'Montserrat', sans-serif;", nrow(wordcloud_data)),
                div(class = "stat-label", style = "font-size: 1rem; color: #9CA3AF; font-family: 'Montserrat', sans-serif;", "Unique Words")
              ),
              div(class = "stat-card", style = "text-align: center;",
                div(class = "stat-value", style = "font-size: 2rem; font-weight: bold; color: #EC4899; font-family: 'Montserrat', sans-serif;", 
                    paste0("\"", wordcloud_data$word[1], "\"")),
                div(class = "stat-label", style = "font-size: 1rem; color: #9CA3AF; font-family: 'Montserrat', sans-serif;", "Most Common Word")
              )
            )
          )
        )
        ...
        div(style = "text-align: center; padding: 4rem;",
          div(style = "font-size: 3rem; margin-bottom: 1rem;", "☁️"),
          p("Unable to generate wordcloud from your song titles", 
            style = "color: #D1D5DB; font-size: 1.2rem; font-family: 'Montserrat', sans-serif;")
        )
      }
    } else {
      div(style = "text-align: center; padding: 4rem;",
        div(style = "font-size: 3rem; margin-bottom: 1rem;", "☁️"),
        p("No words found in your song titles for wordcloud", 
          style = "color: #D1D5DB; font-size: 1.2rem; font-family: 'Montserrat', sans-serif;")
      )
    }
  }
  ...
  # --- SONG WORDCLOUD ---
  output$song_wordcloud <- renderWordcloud2({
    # Check if data is loaded and we have tracks
    if (!values$data_loaded || is.null(values$top_tracks)) {
      return(NULL)
    }

    # Extract tracks data (same logic as in the slide)
    tracks <- list()
    if (!is.null(values$top_tracks$items)) {
      tracks <- values$top_tracks$items
    } else if (!is.null(values$top_tracks) && is.list(values$top_tracks)) {
    ...
    for (i in seq_along(tracks)) {
      ...
      if (!is.null(track$name)) {
        ...
        # Remove common stop words
        stop_words <- c("the", "and", "you", "are", "for", "with", "not", "have", "this", "that", "was", "but", "his", "her", "she", "him", "can", "had", "what", "one", "our", "out", "day", "get", "use", "man", "new", "now", "way", "may", "say", "all", "any", "can", "had", "has", "have", "he", "in", "is", "it", "its", "of", "on", "that", "the", "to", "were", "will", "with")
        words <- words[!tolower(words) %in% stop_words]
    ...
    if (length(song_names) > 0) {
      ...
      if (length(word_freq) > 0) {
        ...
        # Sort by frequency and take top words
        wordcloud_data <- wordcloud_data[order(-wordcloud_data$freq), ]
        wordcloud_data <- wordcloud_data[1:min(80, nrow(wordcloud_data)), ]
        
        # Create wordcloud with Spotify-themed colors
        tryCatch({
          wordcloud2(
            data = wordcloud_data,
            size = 0.8,
            color = rep_len(c("#1DB954", "#1ed760", "#ffffff", "#b3b3b3", "#1f1f1f"), nrow(wordcloud_data)),
          ...
          cat("Error creating wordcloud:", e$message, "\n")
          return(NULL)
        })
      } else {
        return(NULL)
